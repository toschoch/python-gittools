...
def create_hook(dst, env_name, env_path, template, suffix, script_suffix=''):
    ...
    if script_suffix != '':
        ...
        script = pkg_resources.resource_string('gittools',
                                               'git_hooks/{}_{}.{}'.format(template, suffix, script_suffix)).decode(
            'utf-8')
    else:
        script_dst = dst
...
def hook(dst=None, env=None):
    ...
    if platform.platform().lower().startswith('linux'):
        ...
        click.echo("UNIX git hooks successfully installed for environment '{}'!".format(env))

    elif platform.platform().lower().startswith('windows'):

        dst = create_hook(git_dir, env_name=env, env_path=env_dir, template='pre-commit', suffix='win',
                          script_suffix='cmd')
